                        ...
                        >
                            <template v-if="item.row.type === 'toggle-header'">
                                <div
                                    ...
                                    "
                                    @click="
                                        item.row.onClick && item.row.onClick()
                                    "
                                >
                                    <ChevronDown
                                        class="transition-transform duration-200 ease-in-out"
                                        :class="{
                                            '-rotate-90': !item.row.expanded,
                                        }"
                                    />
                                    <span class="ml-1.5">
                                        {{ item.row.label }}
                            ...
                            <template v-else-if="item.row.type === 'me-item'">
                                <ContextMenu>
                                    <ContextMenuTrigger as-child>
                                        <div
                                            class="box-border flex items-center"
                                        >
                                            <div
                                            ...
                                            >
                                                ...
                                                <div
                                                    class="flex-1 overflow-hidden h-9 flex flex-col justify-between"
                                                >
                                                    <div
                                            ...
                                            <TooltipWrapper
                                                ...
                                                :content="
                                                    t(
                                                        'dialog.user.actions.edit_status',
                                                    )
                                                "
                                            >
                                                <Button
                                    ...
                                    <ContextMenuContent>
                                        ...
                                        <ContextMenuSub>
                                            ...
                                            <ContextMenuSubContent>
                                                ...
                                                >
                                                    {{
                                                        t(
                                                            "dialog.gallery_select.none",
                                                        )
                                                    }}
                                                </ContextMenuCheckboxItem>
                                                <ContextMenuSeparator
                                                    v-if="recentStatuses.length"
                                                />
                                                <ContextMenuCheckboxItem
                                                    v-for="(
                                                        item, idx
                                        ...
                                        <ContextMenuSub
                                            v-if="statusPresets.length"
                                        >
                                            <ContextMenuSubTrigger>
                                                {{
                                                    t(
                                                        "dialog.social_status.presets",
                                                    )
                                                }}
                                            </ContextMenuSubTrigger>
                                            <ContextMenuSubContent>
                                        ...
                                        <ContextMenuItem
                                            @click="openEditProfileFromSidebar"
                                        >
                                            {{
                                                t(
                                                    "dialog.user.actions.edit_profile",
                                                )
                                            }}
                                        </ContextMenuItem>
                                    </ContextMenuContent>
                            ...
                            <template
                                v-else-if="item.row.type === 'instance-header'"
                            >
                            ...
                            <template
                                v-else-if="item.row.type === 'friend-item'"
                            >
                                <ContextMenu>
                                    <ContextMenuTrigger as-child>
                                        <FriendItem
                                    ...
                                    <ContextMenuContent>
                                        ...
                                        <ContextMenuItem
                                            ...
                                            "
                                            @click="
                                                friendSendBoop(item.row.friend)
                                            "
                                        >
                                            {{
                                                t(
...
import {
    isActionRecent,
    recordRecentAction,
} from "../../../composables/useRecentActions";
...
const { presets: statusPresets, getStatusClass: presetStatusClass } =
    useStatusPresets();
    ...
    for (const item of friendsInSameInstance.value) {
        for (const friend of item) {
            if (
                ...
                lastLocation.value.friendList.has(friend.id)
            ) {
                ids.add(friend.id);
            }
        }
...
const shouldHideSameInstance = computed(
    () => isSidebarGroupByInstance.value && isHideFriendsInSameInstance.value,
);

const selectedFavoriteGroupKeys = computed(
    () => new Set(sidebarFavoriteGroups.value),
);
...
const onlineFriendsByGroupStatus = computed(() => {
    ...
    const nonFavOnline = onlineFriends.value.filter(
    ...
    return excludeSameInstance(
        [...nonFavOnline, ...unselectedGroupFriends].sort(
...
const vipFriendsDivideByGroup = computed(() => {
    ...
    for (const key in remoteFriendsByGroup) {
        if (Object.hasOwn(remoteFriendsByGroup, key)) {
            ...
            const groupName =
                favoriteFriendGroups.value.find((g) => g.key === key)
                    ?.displayName || "";
            const memberIds = new Set(
                remoteFriendsByGroup[key].map((f) => f.id),
            );
    ...
    for (const { key, groupName, memberIds } of groups) {
        const filteredFriends = visibleFavoriteOnlineFriends.value.filter(
            (friend) => memberIds.has(friend.id),
        );
        if (filteredFriends.length > 0) {
            result.push(
                filteredFriends.map((item) => ({ groupName, key, ...item })),
            );
        }
    }
...
function buildFavoriteRows(rows) {
    const vipFriendCount = isSidebarDivideByFriendGroup.value
        ? vipFriendsDivideByGroup.value.reduce(
            ...
            friendsInSameInstance.value.forEach((friendArr, groupIndex) => {
                ...
                friendArr.forEach((friend, idx) => {
                    rows.push(
                        buildFriendRow(
                            friend,
                            `instance:${groupKey}:${friend?.id ?? idx}`,
                            {
                                isGroupByInstance: true,
                                paddingBottom:
                                    idx === friendArr.length - 1
                                        ? 5
                                        : undefined,
                                itemStyle:
                                    idx === friendArr.length - 1
...
const virtualRows = computed(() => {
    ...
    if (isFriendsGroupMe.value) {
        rows.push({
            type: "me-item",
            key: `me:${currentUser.value?.id ?? "me"}`,
        });
    }
...
function saveFriendsGroupStates() {
    ...
    configRepository.setBool(
        ...
        isOnlineFriends.value,
    );
    configRepository.setBool(
        "VRCX_isFriendsGroupActive",
        isActiveFriends.value,
    );
    configRepository.setBool(
        "VRCX_isFriendsGroupOffline",
        isOfflineFriends.value,
    );
}
...
async function loadFriendsGroupStates() {
    isFriendsGroupMe.value = await configRepository.getBool(
        "VRCX_isFriendsGroupMe",
        true,
    );
    isVIPFriends.value = await configRepository.getBool(
        "VRCX_isFriendsGroupFavorites",
        true,
    );
    isOnlineFriends.value = await configRepository.getBool(
        "VRCX_isFriendsGroupOnline",
        true,
    );
    isActiveFriends.value = await configRepository.getBool(
        "VRCX_isFriendsGroupActive",
        false,
    );
    isOfflineFriends.value = await configRepository.getBool(
        "VRCX_isFriendsGroupOffline",
        true,
    );
...
function toggleSwitchGroupByInstanceCollapsed() {
    isSidebarGroupByInstanceCollapsed.value =
        !isSidebarGroupByInstanceCollapsed.value;
    configRepository.setBool(
        "VRCX_sidebarGroupByInstanceCollapsed",
        isSidebarGroupByInstanceCollapsed.value,
...
const canInviteToMyLocation = computed(() =>
    checkCanInvite(lastLocation.value.location),
);
...
function friendRequestInvite(friend) {
    notificationRequest
        .sendRequestInvite({ platform: "standalonewindows" }, friend.id)
        .then(() => {
...
function friendInvite(friend) {
    ...
    queryRequest
        .fetch("world.location", { worldId: L.worldId })
        .then((args) => {
            notificationRequest
                .sendInvite(
                    {
