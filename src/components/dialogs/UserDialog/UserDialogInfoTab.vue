<template>
    <template
        v-if="
            isFriendOnline(userDialog.friend) ||
            currentUser.id === userDialog.id
        "
    >
        <div
            ...
            <div style="flex: none">
                <template v-if="isRealInstance(userDialog.$location.tag)">
                    <InstanceActionBar
                        ...
                        :refresh-tooltip="
                            t('dialog.user.info.refresh_instance_info')
                        "
                        :on-refresh="
                            () =>
                    ...
                    <template v-if="userDialog.$location.user">
                        ...
                        <div class="flex-1 overflow-hidden">
                            <span
                                ...
                                :style="{
                                    color: userDialog.$location.user
                                        .$userColour,
                                }"
    ...
    <div class="@container">
        <div
            class="grid gap-2.5 grid-cols-1 @[560px]:grid-cols-[minmax(0,1fr)_230px]"
            style="align-items: start"
        >
            <div class="flex flex-col gap-2.5">
                <div class="rounded-xl bg-muted/40 p-3">
                    <div
                        class="flex items-center justify-between mb-2 pb-2 border-b border-border"
                    >
                        <span
                        ...
                        <div class="flex items-center gap-1">
                            ...
                            >
                                ...
                                <Languages
                                    ...
                                    class="h-3 w-3"
                                    :style="{
                                        color: userDialog.theme.iconColor,
                                    }"
                                />
                            </Button>
                            <Button
                    ...
                    <div
                        v-if="
                            userDialog.ref.bioLinks &&
                            userDialog.ref.bioLinks.length
                        "
            ...
            <div class="flex flex-col gap-2.5">
                    ...
                    >
                        ...
                        >
                            <TooltipWrapper
                                ...
                                side="top"
                                :content="
                                    t('dialog.user.info.vrcx_info_tooltip')
                                "
                            >
                                <Info
                    ...
                    <div class="flex flex-col gap-1.5">
                        <template v-if="currentUser.id !== userDialog.id">
                            ...
                            <TooltipWrapper
                                side="top"
                                :content="
                                    t('dialog.user.info.open_previous_instance')
                                "
                            >
                                <div
                        ...
                        >
                            <template #content>
                                <span>{{
                                    formatDateFilter(
                                        userOnlineForTimestamp(userDialog),
                                        "short",
                                    )
                                }}</span>
                            </template>
                            ...
                            >
                                <span class="text-muted-foreground shrink-0">
                                    {{
                                        userDialog.ref.state === "online" &&
                                        userDialog.ref.$online_for
                                ...
                                <span
                                    class="text-right text-muted-foreground"
                                    >{{ userOnlineFor(userDialog.ref) }}</span
                                >
                            </div>
                        </TooltipWrapper>
                        ...
                        <template v-if="currentUser.id !== userDialog.id">
                            <TooltipWrapper
                                side="top"
                                :disabled="
                                    userDialog.dateFriendedInfo.length < 2
                                "
                            >
                                <template #content>
                                    <template
                                        v-for="ref in userDialog.dateFriendedInfo"
                                        :key="ref.type"
                                    >
                                        <span
                                            >{{ ref.type }}:
                ...
                <div class="rounded-xl bg-muted/40 p-3">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        >
                            ...
                            <span
                                class="text-right text-muted-foreground"
                                v-text="userDialog.ref.date_joined"
                            ></span>
                        </div>
                        ...
                        <template v-if="currentUser.id === userDialog.id">
                            <TooltipWrapper side="top">
                                <template #content>
                                    <span>{{
                                        t("view.profile.profile.refresh")
                                    }}</span>
                                </template>
                                <div
                                    class="flex justify-between items-start gap-2 text-xs cursor-pointer hover:text-foreground"
                                ...
                                >
                                    <span
                                        ...
                                        >{{
                                            t(
                                                "view.profile.profile.vrchat_credits",
                                            )
                                        }}</span
                                    >
                                    <span
                ...
                >
                    ...
                    >
                        <span
                            class="truncate"
                            v-text="userDialog.$homeLocationName"
                        ></span>
                        <Button
...
const { hideUserNotes, hideUserMemos } = storeToRefs(
    useAppearanceSettingsStore(),
);
const { bioLanguage, translationApi, translationApiType } = storeToRefs(
    useAdvancedSettingsStore(),
);
...
async function translateBio() {
    ...
    try {
        const providerLabel =
            translationApiType.value === "openai" ? "OpenAI" : "Google";
        const translated = await translateText(
            `${bio}\n\nTranslated by ${providerLabel}`,
            targetLang,
        );
        if (!translated) {
...
function getVRChatCredits() {
    queryRequest
        .fetch("vrchatCredits")
        .then((args) => (vrchatCredit.value = args.json?.balance));
}
