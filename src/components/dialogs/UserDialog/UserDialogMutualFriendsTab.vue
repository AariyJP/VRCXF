<template>
    ...
    >
        <div class="flex items-center">
            ...
            <span class="inline-flex items-center gap-1 ml-1.5 text-sm">
                ...
                {{
                    t("dialog.user.groups.total_count", {
                        count: userDialog.mutualFriends.length,
                    })
                }}
            </span>
        </div>
            ...
            >
                ...
                <SelectContent>
                    <SelectItem
                        v-for="(
                            item, key
                        ) in userDialogMutualFriendSortingOptions"
                        :key="String(key)"
                        :value="String(key)"
                    >
                        {{ t(item.name) }}
    ...
    <ul
        class="mt-2 flex flex-col items-stretch overflow-auto md:flex-row md:flex-wrap md:items-start"
...
const {
    selectedKey: userDialogMutualFriendSortingKey,
    selectByKey: setUserDialogMutualFriendSortingByKey,
} = useOptionKeySelect(
    ...
    processBulk({
        ...
        handle: (args) => {
            for (const json of args.json) {
                if (
                    userDialog.value.mutualFriends.some((u) => u.id === json.id)
                ) {
                    continue;
                }
            ...
            if (success) {
                const mutualIds = userDialog.value.mutualFriends.map(
                    (u) => u.id,
                );
